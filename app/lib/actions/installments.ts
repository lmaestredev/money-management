...
import { z } from 'zod';
import { createInstallment, deleteInstallment, updateInstallment } from '@/app/lib/data/installments';
...
export async function updateInstallmentAction(formData: FormData) {
  ...
  const payBeforeDay = data.pay_before_day ? parseInt(data.pay_before_day, 10) : null;
  const totalPesos = data.monthly_amount_pesos * total;
  const totalDollars = data.monthly_amount_dollars * total;
  ...
  const returnTo = safeDashboardPath(data.return_to, '/dashboard/cuotas');
...
export async function deleteInstallmentAction(formData: FormData) {
  ...
  const redirectTo = safeDashboardPath(parsed.data.redirect_to, '/dashboard/cuotas');
